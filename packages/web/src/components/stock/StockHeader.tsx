  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-3">
        <h1 className="font-display text-2xl font-bold text-white">{quote.symbol}</h1>
        {overview && (
      ...
      <div className="flex items-baseline gap-3">
        ...
        <span className={cn('font-mono text-lg font-semibold', isPositive ? 'text-green-400' : 'text-red-400')}>
      ...
      <div className="flex items-center gap-4 text-xs text-gray-500">
        ...
        <span>Vol: <span className="font-mono text-gray-400">{quote.volume.toLocaleString()}</span></span>
        <span>Prev Close: <span className="font-mono text-gray-400">{formatCurrency(quote.previousClose)}</span></span>
      </div>
    </div>
  );
}
